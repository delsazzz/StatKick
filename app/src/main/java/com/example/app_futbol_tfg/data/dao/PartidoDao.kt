...
interface PartidoDao {
    ...
    @Query("""SELECT * FROM Partidos WHERE id_equipo_local = :idEquipo
            OR id_equipo_visitante = :idEquipo ORDER BY fecha DESC""")
    fun getByEquipo(idEquipo: Int): Flow<List<PartidoEntity>>

    @Query("""SELECT * FROM Partidos WHERE id_competicion = :idCompeticion
            AND id_temporada = :idTemporada ORDER BY fecha ASC""")
    ...
    // ── Cuenta cuántos partidos hay en un estadio — usado en el mapa ───────
    @Query("SELECT COUNT(*) FROM Partidos WHERE id_estadio = :idEstadio")
    fun countByEstadio(idEstadio: Int): Flow<Int>
