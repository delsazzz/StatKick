...
interface UsuarioPartidoDao {
    @Query(
        """SELECT p.* FROM Partidos p INNER JOIN Usuario_Partido up ON up.id_partido = p.id
                WHERE up.id_usuario = :idUsuario ORDER BY p.fecha DESC""")
    ...
    @Query("""SELECT * FROM Usuario_Partido WHERE id_usuario = :idUsuario AND id_partido = :idPartido
                LIMIT 1""")
    ...
    @Query("""SELECT e.id AS id, e.nombre AS nombre, e.escudo AS escudo, COUNT(*) AS vecesVisto
                FROM (SELECT p.id_equipo_local AS id_equipo FROM Usuario_Partido up INNER JOIN Partidos p ON p.id = up.id_partido
                WHERE up.id_usuario = :idUsuario
                UNION ALL
                SELECT p.id_equipo_visitante AS id_equipo FROM Usuario_Partido up INNER JOIN Partidos p ON p.id = up.id_partido
                WHERE up.id_usuario = :idUsuario) equipos_usuario INNER JOIN Equipos e ON e.id = equipos_usuario.id_equipo
                GROUP BY e.id, e.nombre, e.escudo ORDER BY vecesVisto DESC, e.nombre ASC LIMIT :limit""")
    fun getTopEquiposVistos(idUsuario: Int, limit: Int = 10): Flow<List<TeamSeenStat>>
    @Query("""SELECT COUNT(DISTINCT id_equipo) FROM ( SELECT p.id_equipo_local AS id_equipo
                FROM Usuario_Partido up INNER JOIN Partidos p ON p.id = up.id_partido WHERE up.id_usuario = :idUsuario
                UNION
                SELECT p.id_equipo_visitante AS id_equipo FROM Usuario_Partido up INNER JOIN Partidos p ON p.id = up.id_partido
                WHERE up.id_usuario = :idUsuario)""")
    fun countEquiposDistintosVistos(idUsuario: Int): Flow<Int>
    @Query("""SELECT es.id AS id, es.nombre AS nombre, COUNT(*) AS total FROM Usuario_Partido up
                INNER JOIN Partidos p ON p.id = up.id_partido INNER JOIN Estadios es ON es.id = p.id_estadio
                WHERE up.id_usuario = :idUsuario AND p.id_estadio IS NOT NULL GROUP BY es.id, es.nombre
                ORDER BY total DESC, es.nombre ASC LIMIT :limit""")
    fun getEstadiosVistos(idUsuario: Int, limit: Int = 10): Flow<List<StadiumSeenStat>>
    @Query("""SELECT COUNT(DISTINCT p.id_estadio) FROM Usuario_Partido up INNER JOIN Partidos p ON p.id = up.id_partido
