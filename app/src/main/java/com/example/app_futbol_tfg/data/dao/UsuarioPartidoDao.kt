...
interface UsuarioPartidoDao {
    ...
    @Query(
        """
    SELECT e.id AS id, e.nombre AS nombre, e.escudo AS escudo, COUNT(*) AS vecesVisto
    FROM (
    ...
    @Query(
        """
    SELECT COUNT(DISTINCT id_equipo) FROM (
        SELECT p.id_equipo_local AS id_equipo
        FROM Usuario_Partido up
        INNER JOIN Partidos p ON p.id = up.id_partido
        WHERE up.id_usuario = :idUsuario
    ...
    @Query(
        """
    SELECT es.id AS id, es.nombre AS nombre, COUNT(*) AS total
    FROM Usuario_Partido up
    INNER JOIN Partidos p ON p.id = up.id_partido
    ...
    @Query(
        """
    SELECT COUNT(DISTINCT p.id_estadio)
    FROM Usuario_Partido up
    INNER JOIN Partidos p ON p.id = up.id_partido
    WHERE up.id_usuario = :idUsuario
      AND p.id_estadio IS NOT NULL
