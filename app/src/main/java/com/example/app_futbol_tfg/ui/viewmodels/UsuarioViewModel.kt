...
class UsuarioViewModel(private val repository: UsuarioRepository) : ViewModel() {
    suspend fun getUsuarioById(id: Int): UsuarioEntity? = repository.getById(id)
    suspend fun getByUsername(nombreUsuario: String): UsuarioEntity? = repository.getByUsername(nombreUsuario)
    suspend fun getByEmail(email: String): UsuarioEntity? = repository.getByEmail(email)
    fun insertUsuario(
